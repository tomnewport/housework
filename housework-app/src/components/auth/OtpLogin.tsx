...
function getError(error: FetchBaseQueryError | SerializedError | undefined) {
  if (error && "data" in error) {
    ...
    switch (data.detail) {
      case "OTP challenge not open":
        return "Code does not exist.";
      case "OTP challenge expired":
        return "Code has expired.";
      case "OTP challenge not set":
        return "Code does not exist.";
      case "OTP not matched":
        return "Code does not match.";
      case "OTP sub not set":
        return "Code does not exist.";
      default:
        return "Unknown problem with code.";
    }
  }
  return null;
}
...
export default function OtpLogin({
  ...
  const [respondOtp, { error: otpError, reset, isUninitialized }] =
    useRespondOtpMutation();
  ...
  return (
    <>
      <Typography variant="body1" gutterBottom>
        A code has been sent by email to <strong>{sub}</strong>. Please enter it
        to continue.{" "}
        <Link href="#" onClick={onQuit}>
          Click here
        </Link>{" "}
        to try a different email, or log in using a password.
      </Typography>
      {error && (
