...
export default function FloatingActionButton() {
  ...
  const [shortcut, setShortcut] = useState<
    null | "favourites" | "create" | "complete"
  >(null);
  return (
    <>
      <ShortcutFavourites
        open={shortcut === "favourites"}
        handleClose={() =>
          shortcut === "favourites" ? setShortcut(null) : setShortcut(shortcut)
        }
      />
      <SpeedDial
        ariaLabel="Shortcuts"
        icon={<SpeedDialIcon openIcon={<Close />} icon={<Bolt />} />}
        sx={{ position: "fixed", bottom: isLandscape ? 16 : 72, right: 16 }}
      >
        <SpeedDialAction
          onClick={() => setShortcut("favourites")}
          icon={<Star />}
          tooltipTitle="Favourites"
        />
